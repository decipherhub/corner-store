...
라이선스, 시장감시, AML 모니터링, 공시, 분쟁처리의 전체 구현은 후속 단계다.
하지만 Order Book과 RFQ를 지원하려면 최소 운영자 제어면은 MVP 아키텍처에 포함한다.
...
법률상 어떤 라이선스와 책임이 필요한지는 코드가 아니라 법률 검토 결과가 결정한다.
...
있으므로 각 fill의 settlement 직전에 다시 검증한다.
...
일반 ERC-20은 정책이 없는 경우 등록된 public venue로 빠르게 통과시킬 수 있다.
다만 RWA 여부를 단순 매핑 부재로 추론하지 않는다.
...
`deployments/<network>/`의 versioned JSON manifest로 연결한다.

---

## 14. MVP 단계

세 matching engine을 동시에 완성하지 않는다.

### Phase 0 - Legal-to-Technical Matrix

- 법률 요건별 lifecycle stage, actor, data, enforcement point 기록
- 자산·프로그램별 허용 venue 결정
- 미확정 사항을 `UNKNOWN`으로 명시

### Phase 1 - 공통 Policy와 Execution 기반

- ExecutionTypes, ComplianceTypes
- TokenPolicyRegistry
- VenueRegistry, VenueSelector
- OperatorRegistry
- ComplianceEngine
...
- `tools/deploy-v3` 기반 Uniswap v3 배포
- UniswapV3Adapter
- Pool CREATE2 사전 등록
- Mock ERC-3643 AMM swap

### Phase 3 - RFQ

- EIP-712 quote
- nonce, expiry, taker binding
- settlement-time compliance
- 대량 거래 threshold 정책

### Phase 4 - Order Book

- matching 방식 결정
- signed order, cancellation, partial fill
- settlement-time compliance
- operator와 surveillance event
...
- 법률 요건을 policy와 enforcement point로 변환하는 decision matrix
...
- Reg D 관련 초기 Element/Recipe
- Mock ERC-3643 토큰
- AMM 정상·거부 E2E 테스트
- RFQ 인터페이스와 request/quote schema
...
- AMM callback origin
- RFQ signature, nonce, taker, expiry
- Order Book cancellation과 partial fill

### Integration

- AMM-only 토큰
- RFQ-only 토큰
- 기준액 이하 AMM, 초과 RFQ 토큰
- order/quote 생성 후 policy 변경
- venue 생성 후 긴급 중단
- 양쪽 IdentityRegistry가 필요한 RWA-RWA pair
...
- Order 총 fill이 원 주문 수량을 초과하지 않음
