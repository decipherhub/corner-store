# ELE.A-11_claim-freshness

# A-11 Claim Freshness — 부품 심층 인수인계 문서 (Walkthrough)

> **이 문서는 무엇인가.** Decipher RWA DEX의 컴플라이언스 부품 중 하나인 Claim Freshness 부품(내부 식별자 A-11)을, 미국 증권법을 처음 보는 사람도 이해할 수 있도록 풀어 쓴 인수인계 문서다. 개발자·법무팀·외부 자문 변호사·학회원이 각자 작업의 base로 그대로 쓸 수 있도록 — ① 이 규제가 어디서 왔고 왜 존재하는지, ② 어떤 사실을 입력받아 ③ 어떤 로직으로 판정하고 ④ 실패하면 어떻게 처리하며 ⑤ 어떤 테스트로 검증하는지를, 기술 요소마다 풀이를 함께 붙여 설명한다.
>
> **자체완결 원칙.** 이 문서는 다른 내부 문서를 열지 않아도 단독으로 이해되도록 작성했다. 인용은 미국 연방법·연방규칙·SEC 발행문서 등 외부 공식 자료(uscode.house.gov·ecfr.gov·sec.gov·govinfo.gov)만 사용한다.

**⚠ 출처·버전 노트.** 본 부품의 인용은 다음 1차 출처를 기준으로 한다 — 17 C.F.R. §230.506(c)(2)는 eCFR 현행본(Title 17, 2026-06-17 조회; 최종 개정 2026-06-08 반영)이며, 핵심 조항 506(c)(2)(ii)(E)(prior-verification 5년 재사용)는 SEC Release No. 33-10884(Facilitating Capital Formation and Expanding Investment Opportunities by Improving Access to Capital in Private Markets, 2020-11-02 채택; 86 FR 3496, 본문 3598; 2021-01-14 공표, 2021-03-15 시행)로 신설됐다. (종전 문서가 인용하던 Release No. 33-10824(85 FR 64234, 2020-10-09)는 accredited investor *정의* 확대 release로, (E)의 출처가 아니다 — 정정.) ICA §2(a)(51)·§3(c)(7)은 15 U.S.C. §80a-2(a)(51)·§80a-3(c)(7)(uscode.house.gov 현행본, "in effect on June 24, 2026")이고, 506(c)의 일반광고·검증 틀은 Release No. 33-9415(2013)와 JOBS Act §201(a)(Pub. L. 112-106)에서 나온다.

> **읽는 사람이 특히 헷갈리기 쉬운 세 가지 정정 포인트**(상세는 본문·부록 B):
>
> - **Claim Freshness(A-11) ≠ Holding Period(C-01).** A-11은 *자격 증명*(AI claim·QP claim)이 거래 시점에 만료됐는지를 본다 — 시계는 증명이 검증된 시점(`verifiedAt`)부터 흐른다. C-01은 *증권 자체*를 Rule 144(d) 보유기간만큼 들고 있었는지를 본다 — 시계는 증권 취득 시점부터 흐른다. 같은 "시간 검사"처럼 보이지만 대상·기산점·근거 조문이 전부 다르다.
> - **AI 5년 vs QP 1년.** AI claim의 5년 상한은 *법규*에 근거한다(Rule 506(c)(2)(ii)(E)). QP claim의 1년 상한은 *법규가 아니라 Decipher 정책*이다 — ICA §3(c)(7)은 취득 시점 기준일 뿐 재검증·만료 조항이 없다(§3.5·§5.3·§12).
> - **(E)의 5년 재사용 ≠ (C)의 3개월.** (C)의 "within the prior three months"는 제3자 확인서가 *발급될 때* 그 확인이 직전 3개월 내 검증에 근거해야 한다는 *발급 시점* 요건이다. (E)의 5년은 이미 검증된 자를 이후에 다시 신뢰할 수 있는 *재사용 기간*이다. A-11이 거래마다 보는 것은 (E)의 재사용 기간이다(§3.4).

**양식 메모.** 이 문서는 A-13·A-08 인수인계 양식(Walkthrough)의 §1~§13 구조·6-필드 조문 블록·플로우차트·서술 관습을 그대로 따른다. A-11은 *기계 판정형(Pattern A) 보조 부품*이라, 자격 실체를 판단하는 A-13/A-08과 달리 *look-through·수동검토(manual review) 축이 없다* — 그 대신 시간 산술의 *결정성*과 `verifiedAt` 신뢰 모델을 다루므로, 해당 절(§5.5·§6.3·§8·§11.4)은 A-13 양식을 A-11의 결에 맞게 변주했다.
...
[output truncated at 50000 of 50342 characters. Pass a larger max_chars (default 50000) to see more, or use read_page with a ref_id to focus on a smaller section.]

Tab Context:
- Executed on tabId: 437007842
- Available tabs:
