...
- Deployment & Operations Layer는 venue 활성화 전에 필요한 identity 등록을
  preflight한다.
- MVP v1의 AMM Pool은 ERC-3643을 보유하므로 발행자 IdentityRegistry에서 필요한
  적격 venue 주소로 등록되어야 한다. 사용자와 Pool이 ERC-3643을 수신하는 각
  방향에서 identity/transfer 조건을 검증하며, 일반 ERC-20은 명시적
  `UNREGULATED` 정책을 사용한다.
