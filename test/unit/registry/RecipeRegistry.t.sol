...
contract RecipeRegistryTest is Test {
    ...
    function test_legacy_id_alias_is_first_write_immutable_across_versions() public {
        ...
        assertEq(reg.recipeOf(key, 1), recipe);
        assertEq(reg.recipeOf(key, 2), v2);
        assertEq(reg.latestRegisteredVersionOf(key), 2, "catalog metadata advances");
    }
    ...
    function test_latestRegisteredVersion_does_not_regress_when_older_version_registered_later() public {
        address v2 = address(new RecipeRegistryRecipeMock(7, 2));
        reg.registerRecipe(7, 2, v2);
        reg.registerRecipe(7, 1, recipe);
        bytes32 key = reg.recipeKeyOf(7);
        assertEq(reg.latestRegisteredVersionOf(key), 2, "catalog metadata remains monotonic by version");
        assertEq(reg.recipeOf(key, 1), recipe);
        assertEq(reg.recipeOf(key, 2), v2);
    }
    ...
    function test_unregistered_returns_zero() public {
        ...
        assertEq(reg.recipeOf(missing, 1), address(0));
        assertEq(reg.latestRegisteredVersionOf(missing), 0);
    }
