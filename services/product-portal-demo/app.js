(() => {
  ...
  function investorQualification() {
    const items = [["신원 확인", "2027-03-31까지 유효"], ["제재 대상 여부 확인", "2026-12-31까지 유효"], ["거주 국가 대한민국", "2027-03-31까지 유효"], ["고액 투자자 인증", "아직 받지 않았습니다"]];
  ...
  function qualificationReady() {
    const items = [["신원 확인", "2027-03-31까지 유효"], ["제재 대상 여부 확인", "2026-12-31까지 유효"], ["거주 국가 대한민국", "2027-03-31까지 유효"], ["고액 투자자 인증", `${state.provider || "한국인증원"} · 2027-09-04까지 유효`]];
    return shell("investor", "trade", `
      <button class="back" data-route="investor/asset">← ABC 사모 펀드 토큰</button>${header("거래 자격 신청", "이 자산에 필요한 인증을 확인합니다")}${stepper(["인증 확인", "신청서 제출", "운용사 검토"], 1)}<section class="two-column"><div><div class="card requirement-card"><div class="card-heading"><strong>필요한 인증</strong><span>4 / 4 보유</span></div>${items.map(([item, detail]) => `<div class="check-row"><span class="checked-box">✓</span><span><strong>${item}</strong><small>${detail}</small></span></div>`).join("")}</div><label class="card message-field"><span>전할 말</span><textarea id="qualification-message" placeholder="운용사에 전할 말 (선택)"></textarea></label></div><aside class="side-card application-summary"><div class="side-heading">신청 준비</div><div class="alert success compact"><strong>○</strong><span>신청할 수 있습니다</span></div><div class="mini-row"><span>자산</span><strong>ABC 사모 펀드 토큰</strong></div><div class="mini-row"><span>보낼 인증</span><strong>4개</strong></div><div class="mini-row"><span>확인 주체</span><strong>운용사</strong></div><button class="button primary full" data-action="submit-qualification">신청서 제출</button></aside></section>`);
  }
  ...
  function qualificationApproved() {
    return shell("investor", "trade", `${header("거래 자격 신청")}${stepper(["인증 확인", "신청서 제출", "운용사 검토"], 2)}<section class="two-column"><div class="card centered-review approved-review"><div class="success-ring">○</div><h2>거래 자격을 받았습니다</h2><p>이제 ABC 사모 펀드 토큰을 거래할 수 있습니다</p><button class="button primary" data-route="investor/order">주문하러 가기</button></div><aside class="side-card review-status"><div class="side-heading">진행 상태</div><h2 class="positive">승인 완료</h2><div class="status-history"><span>신청서 제출 · 14:12</span><span>운용사 접수 · 14:12</span><strong>승인 · 14:12</strong></div></aside></section>`);
  }
  ...
  function investorOrder(stage = "order") {
    ...
    return shell("investor", "trade", `
      <button class="back" data-route="investor/asset">← ABC 사모 펀드 토큰</button>${header("주문하기")}
      <section class="two-column"><div class="card form-card order-form ${valid ? "" : "is-invalid"}"><div class="card-heading"><strong>수량</strong></div><label class="figma-amount"><input id="order-amount" type="number" min="0" max="600" step="10" value="${state.orderAmount}"/><span>주</span></label><input class="range" id="order-range" type="range" min="0" max="600" step="10" value="${state.orderAmount}" aria-label="주문 수량"/><div class="range-label"><span>0</span><span>600</span></div>${valid ? "" : '<div class="alert danger compact"><strong>×</strong><span>최소 50주부터 주문할 수 있습니다</span></div>'}</div><aside class="side-card order-summary"><div class="side-heading">주문 정보</div><div class="alert ${valid ? "success" : "danger"} compact"><strong>${valid ? "○" : "×"}</strong><span>${valid ? (quoteReady ? "주문할 수 있습니다" : "견적을 요청할 수 있습니다") : "수량이 부족합니다"}</span></div><div class="mini-row"><span>수량</span><strong>${state.orderAmount}주</strong></div>${quoteReady ? `<div class="mini-row"><span>단가</span><strong>100,000 원</strong></div><div class="mini-row"><span>수수료</span><strong>${fee.toLocaleString("ko-KR")} 원</strong></div><div class="mini-row"><span>총 대금</span><strong>${total.toLocaleString("ko-KR")} 원</strong></div><div class="quote-expiry">이 견적은 18초 뒤 만료됩니다</div><div class="verification-line"><span>✓ 3개 딜러 비교</span><span>✓ EIP-712 서명 로컬 검증</span><span>✓ inventory reserved</span></div>` : valid ? '<div class="mini-row"><span>단가</span><strong>—</strong></div><div class="mini-row"><span>총 대금</span><strong>—</strong></div>' : '<div class="mini-row"><span>필요 수량</span><strong>50주 이상</strong></div>'}<button class="button primary full" data-action="${quoteReady ? "accept-quote" : "request-quote"}" ${valid ? "" : "disabled"}>${quoteReady ? "주문하기" : "견적 요청"}</button></aside></section>`);
  }

  function quoteLoadingScreen() {
    return shell("investor", "trade", `<button class="back" data-route="investor/asset">← ABC 사모 펀드 토큰</button>${header("주문하기")}<section class="two-column"><div class="card form-card order-form"><div class="card-heading"><strong>수량</strong></div><label class="figma-amount"><input value="${state.orderAmount}" disabled/><span>주</span></label><input class="range" type="range" min="0" max="600" value="${state.orderAmount}" disabled/><div class="range-label"><span>0</span><span>600</span></div></div><aside class="side-card order-summary dealer-wait"><div class="side-heading">주문 정보</div><div class="spinner"></div><strong>딜러에게 값을 묻는 중</strong><div class="mini-row"><span>수량</span><strong>${state.orderAmount}주</strong></div><div class="mini-row"><span>단가</span><strong>—</strong></div><div class="mini-row"><span>총 대금</span><strong>—</strong></div><button class="button primary full" disabled>주문하기</button><small>Han River Markets · Atlas Liquidity · Seoul Digital</small></aside></section>`);
  }

  function fillLoadingScreen() {
    const fee = Math.round(state.orderAmount * 100000 * 0.001);
    const total = state.orderAmount * 100000 + fee;
    return shell("investor", "trade", `<section class="center-state processing-state"><div class="spinner"></div><h1>주문을 처리하고 있습니다</h1><div class="review-summary"><div><span>자산</span><strong>ABC 사모 펀드 토큰</strong></div><div><span>수량</span><strong>${state.orderAmount}주</strong></div><div><span>총 대금</span><strong>${total.toLocaleString("ko-KR")} 원</strong></div><div class="technical-proof"><span>Protected settlement</span><strong>서명·컴플라이언스·finality 확인 중</strong></div></div></section>`);
  }
  ...
  function render() {
    ...
    if (current === "investor/qualification-ready" && !state.qualificationChecks[3]) {
