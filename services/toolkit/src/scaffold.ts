...
export function scaffoldRFQIntegration(target: string, options: ScaffoldOptions): ScaffoldResult {
  ...
  const files = generatedFiles(manifest, sdkDependency, options.sdkSourceRoot !== undefined);
...
function generatedFiles(
  ...
  const files: Record<string, string> = {
    ...
    "package.json": json(packageManifest(manifest, sdkDependency)),
    "tsconfig.json": json({
      compilerOptions: {
        target: "ES2020",
        module: "CommonJS",
    ...
    "src/index.ts": manifest.mode === "reference-service" ? referenceServiceSource() : existingBackendSource()
  };
...
function packageManifest(manifest: RFQIntegrationManifest, sdkDependency: string) {
  const dependencies: Record<string, string> = {
    [manifest.sdk.package]: sdkDependency
  };
  if (manifest.mode === "reference-service") dependencies.ethers = "^6.13.5";
  return {
...
function envExample(manifest: RFQIntegrationManifest): string {
  const common = [
    "RFQ_CHAIN_ID=31337",
    "RFQ_ADAPTER_ADDRESS=0x0000000000000000000000000000000000000000",
...
3. ${manifest.mode === "reference-service" ? "Start the minimal reference HTTP service with `npm start`." : "Import `createCornerStoreRFQ` into the existing backend request handler."}
