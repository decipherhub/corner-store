...
- **My RFQs** keeps every quote requested during the browser session, lets the
  user reopen the exact taker-bound signed payload, and separately marks quoted,
  settled, rejected and expired requests. Each selected live quote is compared
  with visibly disabled preview fixtures before settlement.
...
Maker approval-revocation proof, targeted temporal-eligibility proof and
transaction-history screens. The Maker screen is not multi-maker onboarding or
dealer administration. These controls send real local-chain transactions or
advance local Anvil time; they are not cosmetic browser state.
...
preserving the global freshness policy and every other investor claim. It issues
a quote while that target remains eligible, advances Anvil beyond the target's
remaining lifetime, and then shows the same still-live quote fail the Router's
latest-policy check. Other eligible investors remain usable as the control
group.
