...
/// @dev SKELETON: emit authority is intentionally OPEN (anyone can call). These
/// are regulatory-weight audit events, so off-chain indexers must NOT trust them
/// by emitter alone. Before production wiring, gate behind `onlyOperator`
/// (Governed) or have the engine/router emit `Events` directly.
